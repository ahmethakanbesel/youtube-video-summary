...
type InMemTranscriptRepository struct {
	logger    *slog.Logger
	cache     map[string]*youtube.TranscriptResponse
	cacheLock sync.RWMutex
}

var _ Transcript = (*InMemTranscriptRepository)(nil)

func NewInMemTranscript(logger *slog.Logger) *InMemTranscriptRepository {
	if logger == nil {
		logger = slog.Default()
	}

	return &InMemTranscriptRepository{
		logger: logger,
		cache:  make(map[string]*youtube.TranscriptResponse),
	}
}

func (r *InMemTranscriptRepository) Get(ctx context.Context, videoID string) (*youtube.TranscriptResponse, error) {
	if videoID == "" {
...
func (r *InMemTranscriptRepository) Save(ctx context.Context, videoID string, transcript *youtube.TranscriptResponse) error {
	if videoID == "" {
...
func (r *InMemTranscriptRepository) Clear(ctx context.Context) error {
	r.cacheLock.Lock()
	defer r.cacheLock.Unlock()
...
func (r *InMemTranscriptRepository) Size() int {
	r.cacheLock.RLock()
	defer r.cacheLock.RUnlock()
	return len(r.cache)
}
